...
# from services.dedup import dedup_questions  # dedup.pyもプロジェクトに含める必要があります
...
#     q_lines = []
#     for q in questions:
#         pr = int(float(q.get("priority", 0.5)) * 100)
...
#             try:
#                 q_list = json.loads(q_list_candidate)
#             except json.JSONDecodeError:
#                 q_list = []
...
#             # Difyからのscore(0-100)を、正規化せずそのままpriorityとして保存
#             try:
#                 priority = float(it.get("score", 50.0))
...
#             st.session_state.dify_last_ran_at = datetime.now().strftime("%H:%M:%S")
            
#             new_qs = _parse_dify_output(resp)
#             if new_qs:
#                 # 既存のUIで追加された質問とマージし、重複排除する
#                 current_qs = st.session_state.get("questions", [])
#                 user_added_qs = [q for q in current_qs if q.get("source") == "ui"]
#                 combined_qs = new_qs + user_added_qs
#                 st.session_state.questions = dedup_questions(combined_qs, threshold=0.9)
#                 st.session_state.questions_source = "dify"
#                 _status(f"{len(new_qs)}件の質問を反映しました")
...
#             dify_mark_next_after(10.0) # クールダウンを10秒に設定
...
def collect_inputs_for_dify() -> Dict[str, str]:
    ...
    # shodan_phase = st.session_state.get("shodan_phase", "")
