...
def call_dify(
    ...
    #         etype = pending_event or evt.get("event") or evt.get("type") or ""
    ...
    #         # 部分データの副作用（本文や部分出力の蓄積）
    #         _on_stream_event_side_effects(etype, evt)

    #         if etype in ("workflow_finished", "message_end"):
    #             final_data = evt.get("data") or {}
    #             break
...
#     """
#     Difyのレスポンスから質問リストを抽出する。
#     - 今回の形式: data.outputs.q_list
#     """
#     try:
...
#         print("parse_dify_output error:", e)
#         return []

# 既存の _parse_dify_output を置き換え
def _parse_dify_output(resp: dict) -> List[Dict]:
    """
    data.outputs.q_list を最優先でパースする。
    q: 質問文, score: 0-100（正規化せず priority として使用）, tag: タグ配列
    """
    try:
        ...
        for it in q_list:
            ...
            if not text:
                continue
            score = it.get("score", 50)  # ← 正規化なしでそのまま
            tags = it.get("tag") or it.get("tags") or []
            if isinstance(tags, str):
                tags = [tags]
            out.append({"text": text, "priority": score, "tags": tags, "status": "unanswered"})
        # デバッグ用
...
def run_dify_once_async(trigger: str):
    ...
    # --- ここが重要：スレッドに ScriptRunContext を付与 ---
...
tab_assist, tab_trans, tab_backlog, tab_profile, tab_history, tab_dify = st.tabs(
    ["Assist（メモ×Top3×編集）", "Transcript（見やすい表示）", "Backlog / 取得状況", "Lead Profile（DB保存）", "履歴 / DB", "Dify デバッグ"]
)
...
with tab_assist:
    ...
    with right:
        ...
        top3 = sorted(candidates, key=lambda x: _to_int_priority(x.get("priority", 0)), reverse=True)[:3]
